...
abstract class BaseTerm extends BaseObject  implements Persistent {
  ...
  const PEER = 'TermPeer';
	...
	public function setTitle($v)
	{
		...
		if ($this->title !== $v) {
			$this->title = $v;
			$this->modifiedColumns[] = TermPeer::TITLE;
		}
	...
	public function setCount($v)
	{
		...
		if ($this->count !== $v) {
			$this->count = $v;
			$this->modifiedColumns[] = TermPeer::COUNT;
		}
	...
	public function setCategory($v)
	{
		...
		if ($this->category !== $v || $v === false) {
			$this->category = $v;
			$this->modifiedColumns[] = TermPeer::CATEGORY;
		}
	...
	public function hasOnlyDefaultValues()
	{
			...
			if (array_diff($this->modifiedColumns, array(TermPeer::CATEGORY))) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->category = ($row[$startcol + 4] !== null) ? (boolean) $row[$startcol + 4] : null;
			$this->resetModified();
			...
			return $startcol + 5; // 5 = TermPeer::NUM_COLUMNS - TermPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(TermPeer::TITLE)) $criteria->add(TermPeer::TITLE, $this->title);
		if ($this->isColumnModified(TermPeer::SLUG)) $criteria->add(TermPeer::SLUG, $this->slug);
		if ($this->isColumnModified(TermPeer::COUNT)) $criteria->add(TermPeer::COUNT, $this->count);
		if ($this->isColumnModified(TermPeer::CATEGORY)) $criteria->add(TermPeer::CATEGORY, $this->category);
	 ...
	 * related Plugins from storage. If this Term is new, it will return
		...
		if ($this->collPlugins === null) {
			if ($this->isNew()) {
			   $this->collPlugins = array();
				...
				$criteria->add(PluginPeer::CATEGORY_ID, $this->id);
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Term has previously been saved, it will retrieve
	 * related TermRelationships from storage. If this Term is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collTermRelationships === null) {
			if ($this->isNew()) {
			   $this->collTermRelationships = array();
				...
				$criteria->add(TermRelationshipPeer::TERM_ID, $this->id);
	 ...
	 * actually need in Term.
	 */
	public function getTermRelationshipsJoinPlugin($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(TermPeer::DATABASE_NAME);
		}
		...
		if ($this->collTermRelationships === null) {
			if ($this->isNew()) {
				...
				$this->collTermRelationships = TermRelationshipPeer::doSelectJoinPlugin($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
